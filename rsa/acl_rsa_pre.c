                   ...
                   vect2 e, vect p, vect q, vect6 tmp, size_t len)
{
    ...
    if (!acl_p_coprime(e, phi, tmp, 2*len)) {
        return FALSE;
    }
    ...
    if (dmp1) {
        acl_p_mod(dmp1, d, 2*len, p, len);          // dmp1 = d mod (p - 1)
    }
    if (dmq1) {
        acl_p_mod(dmq1, d, 2*len, q, len);          // dmq1 = d mod (q - 1)
    }
    p[0]++; q[0]++;                                 // p = p + 1, q = q + 1
    if (iqmp) {
        acl_p_mod_inv(iqmp, q, 0, p, tmp, len);     // iqmp = q^(-1) mod p
    }
